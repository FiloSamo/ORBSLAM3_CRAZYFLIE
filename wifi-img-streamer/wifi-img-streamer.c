...
void rx_task(void *parameters)
{
  while (1)
  {
    ...
    cpxReceivePacketBlocking(CPX_F_WIFI_CTRL, &rxp); // Wait for a packet from the WiFi host
...
void rx_from_wifi(void *parameters)
{
  while (1)
  {
    cpxReceivePacketBlocking(CPX_F_APP, &wifipacket); // Wait for a packet from the WiFi host

    WiFiCTRLPacket_t * wifiCtrl = (WiFiCTRLPacket_t*) wifipacket.data;
...
{
  uint32_t timestamp;
} __attribute__((packed)) delay_header_t;

void sendTimePacket(uint32_t timestamp) {
    CPXPacket_t timePacket;
    delay_header_t* tp = (delay_header_t*) timePacket.data;

    tp->timestamp = timestamp;

    timePacket.dataLength = sizeof(delay_header_t);
...
#ifdef SETUP_WIFI_AP
void setupWiFi(void) {
  static char ssid[] = "WiFi-Crazyflie";
...
void start_example(void)
{
  ...
  xTask = xTaskCreate(rx_from_wifi, "rx_from_wifi", configMINIMAL_STACK_SIZE * 2,
                      NULL, tskIDLE_PRIORITY + 1, NULL);
