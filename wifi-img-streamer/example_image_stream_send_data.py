...
def build_cpx_packet(rsv, version, function, source, destination, last_packet, data):
...
def send_position(client_socket, x, y, z, roll, pitch, yaw, timestamp):

  payload = struct.pack('>B7i', WIFI_POSITION_SENDED, x, y, z, roll, pitch, yaw, timestamp)
  print("Sending payload of length: {}".format(len(payload)))

  full_packet = build_cpx_packet(rsv = 0,
    version = 0,
...
while(1):
    ...
    if magic == 0xBC:
      ...
      packetInfoRaw = rx_bytes(4)
      [length, routing, function] = struct.unpack('<HBB', packetInfoRaw)
      timestamp_data = rx_bytes(length - 2)
    ...
    send_position(client_socket, 1, 1, 1, 1, 1, 1, timestamp[0])
